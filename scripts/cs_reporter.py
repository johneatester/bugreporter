...
SLACK_CHANNEL = "cs-team-bug-task-reported"
STATE_FILE = Path("state/cs_state.json")

# Re-scan window: every run also re-checks this far back and dedups via
# reported_ids, so issues near the boundary (clock skew, timezone edges, issues
...
def load_state() -> tuple[str, list]:
    ...
    if STATE_FILE.exists():
        try:
            ...
            print(f"[CS] WARNING: state file unreadable ({e}); defaulting to 24h lookback")
...
def query_since(last_checked: str) -> str:
    ...
    try:
        ...
        print(f"[CS] WARNING: bad last_checked '{last_checked}'; defaulting to 24h lookback")
        return _default_since()
...
def fetch_issues(since: str) -> list:
    reporters_jql = ", ".join(f'"{r}"' for r in CS_REPORTERS)
    ...
    blocks = [
        {
            ...
            "text": {
                "type": "plain_text",
                "text": f":memo: {count} New Issue{'s' if count != 1 else ''} Reported",
            },
        },
        {"type": "divider"},
    ]
...
def post_to_slack(issues: list) -> None:
    resp = requests.post(
        ...
        json={
            ...
            "text": f":memo: {len(issues)} new issue(s) reported in Jira",
...
def main() -> None:
    ...
    print(f"[CS] Checking for issues since: {since} (last_checked={last_checked})")
    all_issues = fetch_issues(since)
    new_issues = [i for i in all_issues if i["key"] not in reported_set]

    print(f"[CS] Found {len(all_issues)} issue(s), {len(new_issues)} not yet reported")
    ...
    if new_issues:
        ...
        # skipping issues that were never delivered.
        post_to_slack(new_issues)
        reported_ids.extend(i["key"] for i in new_issues)
        print(f"[CS] Posted {len(new_issues)} issue(s) to #{SLACK_CHANNEL}")

    save_state(now, reported_ids)
    print(f"[CS] State saved: {now}")
